...
import com.arcabank.core_finance.notificator.event.PaymentFailedEvent;
...
@NotificationHandler(event = PaymentFailedEvent.class)
public class PaymentFailedStrategy implements NotificationStrategy<PaymentFailedEvent> {

    @Override
    public BaseNotification buildNotification(PaymentFailedEvent event) {
        BaseNotification notification = new BaseNotification();
        notification.setId(UUID.randomUUID());
        notification.setUserId(event.getUserId());
        notification.setNotificationType(NotificationType.PAYMENT_FAILED);
        notification.setActionType(ActionType.NAVIGATE);
        notification.setUserEmail(event.userEmail());
        notification.setTitle(TemplateTitleCreator.generatePaymentFailedTitle());
        notification.setText(TemplateTextCreator.generatePaymentFailedText(event.amount(), event.currency(), event.reason()));
